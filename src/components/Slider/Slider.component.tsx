...
export const Slider = forwardRef<HTMLInputElement, SliderProps>(
  (
    ...
    const clamp = (v: number) => (blockedRange && v > blockedRange.min ? blockedRange.min : v);

    const isControlled = value !== undefined;
    const [localValue, setLocalValue] = useState<number>(clamp(defaultValue ?? minVal));
      ...
      <div className={rootClasses}>
        {(label || showValue) && (
          <div className="eidos-slider-top">
            ...
            {showValue && <span className="eidos-slider-value">{fmt(currentValue)}</span>}
          </div>
        )}
