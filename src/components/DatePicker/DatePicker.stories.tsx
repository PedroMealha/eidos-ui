...
export const Default: Story = {
  ...
  render: ({ mode, value: _v, onChange: _oc, ...rest }) => {
    ...
    const [range,    setRange]    = useState<DateTimeValue<"range">>({ date: { start: null, end: null } });
        ...
        <div style={{ marginTop: "12px", fontSize: "13px", color: "#666" }}>
          ...
          {value.date.length > 0 && (
            <ul style={{ marginTop: "6px", paddingLeft: "20px" }}>
              {value.date.map((d, i) => <li key={i}>{d}</li>)}
            </ul>
          )}
        </div>
      </div>
...
export const DateConstraints: Story = {
  parameters: {
    docs: {
      description: {
        story: "Restrict the selectable range with `minDate` / `maxDate`. Dates outside the range are disabled in the calendar.",
      },
    },
  },
  render: () => {
