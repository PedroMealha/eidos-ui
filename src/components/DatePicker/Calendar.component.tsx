...
export const Calendar: React.FC<CalendarProps> = ({
	...
	const calendarDays = useMemo(() => {
		...
		for (let i = 0; i < 42; i++) {
			...
			const isInRange =
				rangeStart && rangeEnd && current.isAfter(rangeStart, 'day') && current.isBefore(rangeEnd, 'day');
	...
	const weekDays = useMemo(() => {
		const days = [];
		let start = firstDayOfWeek === 0 ? dayjs().startOf('week') : dayjs().startOf('week').add(1, 'day');
